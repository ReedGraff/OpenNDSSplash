...
* change opennds login setting to point to the theme we're going to alter: `uci set opennds.@opennds[0].login_option_enabled='2'` & `uci commit opennds` & `/etc/init.d/opennds restart`
...
* view logs in tmp/ `cat /tmp/portal_auth.json`
